...
export async function invokeMock<T>(cmd: string, args?: any): Promise<T> {
  ...
  switch (cmd) {
    ...
    case "redis_list_databases":
      return mockRedisListDatabases(args.id) as Promise<T>;

    case "redis_scan_keys":
      return mockRedisScanKeys(args) as Promise<T>;

    case "redis_get_key":
      return mockRedisGetKey(args.id, args.database, args.key) as Promise<T>;

    case "redis_set_key":
      return mockRedisSetKey(args.id, args.database, args.payload) as Promise<T>;

    case "redis_delete_key":
      return mockRedisDeleteKey(args.id, args.database, args.key) as Promise<T>;

    case "redis_rename_key":
      return mockRedisRenameKey(args.id, args.database, args.oldKey, args.newKey, args.force) as Promise<T>;

    case "redis_set_ttl":
      return mockRedisSetTtl(args.id, args.database, args.key, args.ttlSeconds) as Promise<T>;

    case "redis_server_info":
      return mockRedisServerInfo(args.id, args.database) as Promise<T>;

    case "redis_server_config":
      return mockRedisServerConfig(args.id, args.database) as Promise<T>;

    case "redis_slowlog_get":
      return mockRedisSlowlogGet(args.id, args.database, args.count) as Promise<T>;

    case "redis_execute_raw":
      return mockRedisExecuteRaw(args.id, args.database, args.command) as Promise<T>;
    ...
    case "mcp_configure_client":
      return mockMcpConfigureClient(args.clientName) as Promise<T>;

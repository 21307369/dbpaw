...
pub(crate) enum MssqlStatementKind {
    Read,
    AffectedRows,
    Other,
}
...
fn classify_mssql_statement(sql: &str) -> MssqlStatementKind {
    let Some(first_keyword) = super::super::first_sql_keyword(sql) else {
    ...
    match first_keyword.as_str() {
        ...
        "EXEC" | "EXECUTE" => {
            ...
            match extract_sp_executesql_text(sql)
                .and_then(|inner| super::super::first_sql_keyword(&inner))
                .as_deref()
            {
...
impl MssqlDriver {
    pub(crate) async fn fetch_rows(&self, sql: &str) -> DriverResult<Vec<Row>> {
        Ok(self.fetch_rows_with_columns(sql).await?.0)
    }

    pub(crate) async fn fetch_rows_with_columns(
        &self,
        sql: &str,
    ...
    pub(crate) async fn fetch_query_result_json(
        &self,
        sql: &str,
...
mod tests {
    ...
    fn test_normalize_mssql_row_json_stringify_high_precision() {
        let mut row =
            serde_json::json!({"id": 9223372036854775807_i64, "amount": 1234.56, "name": "x"});
    ...
    fn test_classify_mssql_statement_distinguishes_read_and_affected_rows() {
        assert_eq!(
            classify_mssql_statement("SELECT 1 AS ok"),
            MssqlStatementKind::Read
        );
        assert_eq!(
            classify_mssql_statement("INSERT INTO t(id) VALUES (1)"),
            MssqlStatementKind::AffectedRows
        );
        assert_eq!(
            classify_mssql_statement("CREATE TABLE t(id INT)"),
            MssqlStatementKind::Other
        );
    }
    ...
    fn test_extract_sp_executesql_text_unescapes_doubled_quotes() {
        assert_eq!(
            extract_sp_executesql_text("EXEC sp_executesql N'SELECT N''alice'' AS name'")
                .as_deref(),
    ...
    fn test_build_for_json_query_trims_trailing_semicolon() {
        assert_eq!(
            build_for_json_query("SELECT id, name FROM dbo.users;"),
    ...
    fn test_build_for_json_query_preserves_existing_for_json() {
        assert_eq!(
            build_for_json_query("SELECT 1 FOR JSON PATH, ROOT('data')"),

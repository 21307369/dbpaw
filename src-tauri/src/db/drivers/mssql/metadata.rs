...
use crate::models::{ColumnInfo, ColumnSchema, ForeignKeyInfo, IndexInfo, RoutineInfo, SchemaForeignKey, SchemaOverview, SynonymInfo, TableInfo, TableMetadata, TableSchema, TableStructure};
...
pub(crate) fn mssql_full_type_string(data_type: &str, max_length: i64, precision: i64, scale: i64) -> String {
    let dt = data_type.to_ascii_lowercase();
    match dt.as_str() {
        "varchar" | "char" | "varbinary" | "binary" => {
            let len = if max_length == -1 { "MAX".to_string() } else { max_length.to_string() };
        ...
        "datetime2" | "datetimeoffset" | "time" => {
            if scale > 0 { format!("{}({})", data_type, scale) } else { data_type.to_string() }
        }
...
fn render_mssql_create_table_ddl(
    ...
    columns: &[MssqlColumnInfo], key_constraints: &[MssqlKeyConstraint],
    check_constraints: &[(String, String)], foreign_keys: &[ForeignKeyInfo],
    indexes: &[IndexInfo],
) -> String {
    ...
    for col in columns {
        if col.is_computed {
            ...
            lines.push(format!("    {} AS {}", quote_ident(&col.name).unwrap_or_else(|_| col.name.clone()), def));
            continue;
        }
        let mut parts = vec![format!("    {}", quote_ident(&col.name).unwrap_or_else(|_| col.name.clone())), col.full_type.clone()];
        if !col.is_nullable { parts.push("NOT NULL".to_string()); }
        if let Some(ref def) = col.default_definition { parts.push(format!("DEFAULT {}", def)); }
        lines.push(parts.join(" "));
    }

    for kc in key_constraints {
        let col_list: Vec<String> = kc.columns.iter().map(|c| quote_ident(c).unwrap_or_else(|_| c.clone())).collect();
        lines.push(format!("    CONSTRAINT {} {} ({})", quote_ident(&kc.name).unwrap_or_else(|_| kc.name.clone()), kc.constraint_type, col_list.join(", ")));
    }

    for (name, definition) in check_constraints {
        lines.push(format!("    CONSTRAINT {} CHECK ({})", quote_ident(name).unwrap_or_else(|_| name.clone()), definition));
    }

    for fk in foreign_keys {
        let fk_cols: Vec<String> = vec![quote_ident(&fk.column).unwrap_or_else(|_| fk.column.clone())];
        let pk_cols: Vec<String> = vec![quote_ident(&fk.referenced_column).unwrap_or_else(|_| fk.referenced_column.clone())];
        let ref_table = format!("{}.{}", quote_ident(&fk.referenced_schema.as_deref().unwrap_or("dbo")).unwrap_or_else(|_| "dbo".to_string()), quote_ident(&fk.referenced_table).unwrap_or_else(|_| fk.referenced_table.clone()));
        let mut fk_line = format!("    CONSTRAINT {} FOREIGN KEY ({}) REFERENCES {} ({})", quote_ident(&fk.name).unwrap_or_else(|_| fk.name.clone()), fk_cols.join(", "), ref_table, pk_cols.join(", "));
        if let Some(ref on_update) = fk.on_update { fk_line.push_str(&format!(" ON UPDATE {}", on_update)); }
        if let Some(ref on_delete) = fk.on_delete { fk_line.push_str(&format!(" ON DELETE {}", on_delete)); }
        lines.push(fk_line);
    }

    let mut result = format!("CREATE TABLE {}.{} (\n", quote_ident(schema).unwrap_or_else(|_| schema.to_string()), quote_ident(table).unwrap_or_else(|_| table.to_string()));
    result.push_str(&lines.join(",\n"));
    result.push_str("\n);\n");

    for idx in indexes {
        let unique = if idx.unique { "UNIQUE " } else { "" };
        let col_list: Vec<String> = idx.columns.iter().map(|c| quote_ident(c).unwrap_or_else(|_| c.clone())).collect();
        result.push_str(&format!("\nCREATE {}INDEX {} ON {}.{} ({});", unique, quote_ident(&idx.name).unwrap_or_else(|_| idx.name.clone()), quote_ident(schema).unwrap_or_else(|_| schema.to_string()), quote_ident(table).unwrap_or_else(|_| table.to_string()), col_list.join(", ")));
    }
...
impl MssqlDriver {
    pub(crate) async fn load_mssql_columns(&self, schema: &str, table: &str) -> DriverResult<Vec<MssqlColumnInfo>> {
        let sql = format!(
        ...
        let comment_map: HashMap<String, String> = comment_rows.iter().filter_map(|row| {
            let val = Self::parse_string(row, 1);
            if val.is_empty() { None } else { Some((Self::parse_string(row, 0), val)) }
        }).collect();
        ...
        for row in rows {
            ...
            let max_length = if data_type.eq_ignore_ascii_case("nvarchar") || data_type.eq_ignore_ascii_case("nchar") { max_length_raw / 2 } else { max_length_raw };
            ...
            cols.push(MssqlColumnInfo {
                name: name.clone(), data_type, full_type,
                is_nullable: Self::parse_i64(&row, 2) == 1,
                is_identity: Self::parse_i64(&row, 6) == 1,
                is_computed: Self::parse_i64(&row, 7) == 1,
                computed_definition: if computed_def.is_empty() { None } else { Some(computed_def) },
                default_definition: if default_def.is_empty() { None } else { Some(default_def) },
                default_constraint_name: if default_cn.is_empty() { None } else { Some(default_cn) },
    ...
    pub(crate) async fn load_mssql_key_constraints(&self, schema: &str, table: &str) -> DriverResult<Vec<MssqlKeyConstraint>> {
        let sql = format!(
        ...
        for row in rows {
            ...
            map.entry(name).or_insert((type_desc, Vec::new())).1.push((ord, col));
        }
        Ok(map.into_iter().map(|(name, (type_desc, mut cols))| {
            cols.sort_by_key(|(ord, _)| *ord);
            MssqlKeyConstraint { name, constraint_type: if type_desc.contains("PRIMARY") { "PRIMARY KEY".to_string() } else { "UNIQUE".to_string() }, columns: cols.into_iter().map(|(_, c)| c).collect() }
        }).collect())
    }

    pub(crate) async fn load_mssql_check_constraints(&self, schema: &str, table: &str) -> DriverResult<Vec<(String, String)>> {
        let sql = format!(
        ...
        let rows = self.fetch_rows(&sql).await?;
        Ok(rows.into_iter().map(|row| (Self::parse_string(&row, 0), Self::parse_string(&row, 1))).collect())
    }

    pub(crate) async fn load_mssql_foreign_keys(&self, schema: &str, table: &str) -> DriverResult<Vec<ForeignKeyInfo>> {
        let sql = format!(
        ...
        for row in rows {
            fks.push(ForeignKeyInfo {
                name: Self::parse_string(&row, 0), column: Self::parse_string(&row, 1),
                referenced_schema: Some(Self::parse_string(&row, 2)), referenced_table: Self::parse_string(&row, 3),
                referenced_column: Self::parse_string(&row, 4), on_update: Some(Self::parse_string(&row, 5)),
    ...
    pub(crate) async fn load_mssql_indexes(&self, schema: &str, table: &str, include_constraints: bool) -> DriverResult<Vec<IndexInfo>> {
        let constraint_filter = if include_constraints { "" } else { " AND i.is_primary_key = 0 AND i.is_unique_constraint = 0" };
        let sql = format!(
        ...
        for row in rows {
            ...
            let entry = map.entry(name).or_insert((unique, Some(idx_type.clone()), Vec::new()));
            entry.0 = unique;
            if entry.1.is_none() && !idx_type.is_empty() { entry.1 = Some(idx_type); }
            entry.2.push((ord, col));
        }
        let mut indexes: Vec<IndexInfo> = map.into_iter().map(|(name, (unique, index_type, mut cols))| {
            cols.sort_by_key(|(ord, _)| *ord);
            IndexInfo { name, unique, index_type: Some(index_type.unwrap_or_default()), columns: cols.into_iter().map(|(_, c)| c).collect() }
        }).collect();
    ...
    pub(crate) async fn list_databases_impl(&self) -> DriverResult<Vec<String>> {
        ...
        Ok(rows.iter().map(|row| Self::parse_string(row, 0)).filter(|s| !s.is_empty()).collect())
    }

    pub(crate) async fn list_tables_impl(&self, schema: Option<String>) -> DriverResult<Vec<TableInfo>> {
        let schema_filter = schema.filter(|s| !s.trim().is_empty()).map(|s| format!("AND s.name = '{}'", escape_literal(s.trim())));
        let sql = format!(
        ...
        let rows = self.fetch_rows(&sql).await?;
        Ok(rows.into_iter().map(|row| TableInfo { schema: Self::parse_string(&row, 0), name: Self::parse_string(&row, 1), r#type: Self::parse_string(&row, 2) }).collect())
    }

    pub(crate) async fn get_table_structure_impl(&self, schema: String, table: String) -> DriverResult<TableStructure> {
        let pk_sql = format!(
        ...
        let pk_set: HashSet<String> = pk_rows.iter().map(|row| Self::parse_string(row, 0)).collect();

        let mssql_cols = self.load_mssql_columns(&schema, &table).await?;
        let columns = mssql_cols.into_iter().map(|col| ColumnInfo {
            name: col.name.clone(), r#type: col.full_type, nullable: col.is_nullable,
            default_value: col.default_definition, primary_key: pk_set.contains(&col.name),
            comment: col.comment, default_constraint_name: col.default_constraint_name,
        }).collect();
        Ok(TableStructure { columns })
    }

    pub(crate) async fn get_table_metadata_impl(&self, schema: String, table: String) -> DriverResult<TableMetadata> {
        let columns = self.get_table_structure_impl(schema.clone(), table.clone()).await?.columns;
        ...
        Ok(TableMetadata { columns, indexes, foreign_keys, clickhouse_extra: None, cassandra_extra: None, special_type_summaries: vec![] })
    }

    pub(crate) async fn get_table_ddl_impl(&self, schema: String, table: String) -> DriverResult<String> {
        ...
        Ok(render_mssql_create_table_ddl(&schema, &table, &columns, &key_constraints, &check_constraints, &foreign_keys, &indexes))
    }

    pub(crate) async fn get_schema_overview_impl(&self, schema: Option<String>) -> DriverResult<SchemaOverview> {
        ...
        for row in rows {
            ...
            table_map.entry((schema_name, table_name)).or_default().push(ColumnSchema { name: col_name, r#type: col_type });
        }
        let mut tables: Vec<TableSchema> = table_map.into_iter().map(|((schema, name), columns)| TableSchema { schema, name, columns }).collect();
        tables.sort_by(|a, b| a.schema.cmp(&b.schema).then(a.name.cmp(&b.name)));
        Ok(SchemaOverview { tables })
    }

    pub(crate) async fn list_routines_impl(&self, schema: Option<String>) -> DriverResult<Vec<RoutineInfo>> {
        let schema_filter = schema.filter(|s| !s.trim().is_empty()).map(|s| format!("AND s.name = '{}'", escape_literal(s.trim())));
        let sql = format!(
        ...
        let rows = self.fetch_rows(&sql).await?;
        Ok(rows.into_iter().map(|row| RoutineInfo { schema: Self::parse_string(&row, 0), name: Self::parse_string(&row, 1), r#type: Self::parse_string(&row, 2) }).collect())
    }

    pub(crate) async fn get_routine_ddl_impl(&self, schema: String, name: String, routine_type: String) -> DriverResult<String> {
        ...
        let rows = self.fetch_rows(&sql).await?;
        let ddl = rows.first().map(|row| Self::parse_string(row, 0)).unwrap_or_default();
        if ddl.trim().is_empty() {
            return Err(AppError::not_found(format!("Routine '{}.{}' does not exist or its definition is not visible", schema, name)));
        }
        Ok(ddl)
    }

    pub(crate) async fn list_synonyms_impl(&self, schema: Option<String>) -> DriverResult<Vec<SynonymInfo>> {
        let schema_filter = schema.filter(|s| !s.trim().is_empty()).map(|s| format!("AND s.name = '{}'", escape_literal(s.trim())));
        let sql = format!(
        ...
        let rows = self.fetch_rows(&sql).await?;
        Ok(rows.into_iter().map(|row| SynonymInfo { schema: Self::parse_string(&row, 0), name: Self::parse_string(&row, 1), base_object_type: Self::parse_string(&row, 2) }).collect())
    }

    pub(crate) async fn get_schema_foreign_keys_impl(&self, database: Option<&str>) -> DriverResult<Vec<SchemaForeignKey>> {
        let schema_filter = database.filter(|s| !s.trim().is_empty()).map(|s| format!("AND ss.name = '{}'", escape_literal(s.trim())));
        let sql = format!(
        ...
        for row in rows {
            ...
            foreign_keys.push(SchemaForeignKey {
                name: Self::parse_string(&row, 0), source_schema: Some(Self::parse_string(&row, 1)),
                source_table: Self::parse_string(&row, 2), source_column: Self::parse_string(&row, 3),
                target_schema: Some(Self::parse_string(&row, 4)), target_table: Self::parse_string(&row, 5),
...
mod tests {
    ...
    fn quote_ident_allows_common_mssql_names() {
        assert_eq!(quote_ident("order-detail 2026").unwrap(), "[order-detail 2026]");
    ...
    fn test_routine_type_sql_filter_maps_supported_types() {
        ...
        assert_eq!(routine_type_sql_filter("PROCEDURE").unwrap(), "('P')");
        assert_eq!(routine_type_sql_filter("function").unwrap(), "('FN','IF','TF','FS','FT')");
    ...
    fn test_mssql_full_type_string_nvarchar() {
        ...
        assert_eq!(mssql_full_type_string("nvarchar", -1, 0, 0), "nvarchar(MAX)");
    }
    ...
    fn test_mssql_full_type_string_passthrough() {
        ...
        assert_eq!(mssql_full_type_string("uniqueidentifier", 0, 0, 0), "uniqueidentifier");
    }
}

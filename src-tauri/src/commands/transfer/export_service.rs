use super::writer::{extension_for_format, resolve_output_path, ExportWriter};
...
pub(super) async fn export_table_data_direct(
    state: &AppState,
    id: i64,
...
pub(super) async fn export_database_sql_direct(
    state: &AppState,
    id: i64,
    database: String,
    driver: String,
    ...
    let chunk = chunk_size.unwrap_or(DEFAULT_CHUNK_SIZE).max(1);
    super::super::execute_with_retry_from_app_state(state, id, Some(database), |db_driver| {
        let output_path = output_path.clone();
        let driver = driver.clone();
        let format = format.clone();
...
pub(super) async fn export_query_result_direct(
    state: &AppState,
    id: i64,
    database: Option<String>,
    sql: String,

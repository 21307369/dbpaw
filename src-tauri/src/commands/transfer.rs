...
pub async fn export_table_data_direct(
    ...
    export_service::export_table_data_direct(
        state,
        id,
        database,
        schema,
...
pub async fn export_database_sql_direct(
    ...
    export_service::export_database_sql_direct(
        state, id, database, driver, format, file_path, chunk_size,
    )
    .await
}
...
pub async fn export_query_result(
    ...
    export_query_result_direct(state.inner(), id, database, sql, driver, format, file_path).await
}
...
pub async fn export_query_result_direct(
    ...
    export_service::export_query_result_direct(state, id, database, sql, driver, format, file_path)
        .await
}
...
pub async fn import_sql_file(
    ...
    import_sql_file_direct(state.inner(), id, database, file_path, driver).await
}
...
pub async fn import_sql_file_direct(
    ...
    import_service::import_sql_file_direct(state, id, database, file_path, driver).await
}
...
mod tests {
    ...
    use super::export_service::do_database_export;
    use super::import_plan::{
        import_transaction_sql, normalize_driver_name, parse_mssql_batches, parse_sql_statements,
        prepare_import_plan, should_use_outer_import_transaction, statement_controls_transaction,
    };
    use super::import_service::truncate_error_message;
    use super::sql_writer::{quote_ident, quote_target, sql_value};
    use super::writer::{csv_escape, extension_for_format, validate_output_path, ExportWriter};
    ...
    fn transfer_module_boundaries_are_explicit() {
        ...
        let _ = super::sql_writer::quote_target(Some("public"), "users", "postgres");
